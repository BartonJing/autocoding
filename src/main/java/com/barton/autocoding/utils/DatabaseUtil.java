...
import com.barton.autocoding.entity.DatabaseType;
...
public class DatabaseUtil {
    ...
    public static List<ColumnData> getColumnDatas(String tableName) {
        ...
        //List<String> columnComments = getColumnComments(tableName);
        if(columnNames.size() == columnTypes.size() && columnNames.size() == columnComments.size()){
            for(int i=0,l=columnNames.size();i<l;i++){
                ColumnData columnData = new ColumnData(columnNames.get(i),columnTypes.get(i),null,columnComments.get(i));
                DataType dataType = DataType.generateDataType(null);
                columnData.setColumnJdbcType(dataType.getJdbcType());
                columnData.setColumnJavaType(dataType.getJavaType());
                columnData.setColumnJavaName(columnData.getColumnName());
